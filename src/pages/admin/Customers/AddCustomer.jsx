...
const AddOrderPage = () => {
  ...
  const handleUploadImages = async (event) => {
    ...
    try {
      ...
      const imageRef = ref(storage, `orders/${uuidv4()}`);
      const snapshot = await uploadBytes(imageRef, resizedImageBlob);
      const url = await getDownloadURL(snapshot.ref);

      setOrder((prevOrder) => ({
        ...prevOrder,
        images: [...prevOrder.images, url],
      }));
  ...
  // Add color (name and hex) to the order's color array
  const handleAddColor = () => {
    ...
    setOrder((prevOrder) => ({
      ...prevOrder,
      colors: [...prevOrder.colors, { name: colorInput, hex: hexInput }],
    }));
  ...
  const handleRemoveColor = (index) => {
    setOrder((prevOrder) => ({
      ...prevOrder,
      colors: prevOrder.colors.filter((_, i) => i !== index),
    }));
  };

  // Add size to the order's size array
  const handleAddSize = () => {
    if (sizeInput.trim() === "") {
    ...
    setOrder((prevOrder) => ({
      ...prevOrder,
      sizes: [...prevOrder.sizes, sizeInput],
    }));
  ...
  const handleRemoveSize = (index) => {
    setOrder((prevOrder) => ({
      ...prevOrder,
      sizes: prevOrder.sizes.filter((_, i) => i !== index),
    }));
  };
        ...
        <div className="flex flex-row gap-4">
          ...
          <div className="basis-2/3">
            <input
              type="text"
              name="orderVideoUrl"
              value={order.orderVideoUrl}
              onChange={(e) => {
                setOrder({
                  ...order,
                  orderVideoUrl: e.target.value,
                });
              }}
              placeholder="Order Video Link"
              className={`${inputStyles}  w-full`}
            />
          </div>
        </div>
